...
class UploadHelper extends AppHelper {
	...
	public function edit ($model, $id, $formSelector = null) {
		require_once (ROOT . DS . APP_DIR . "/Plugin/AjaxMultiUpload/Config/bootstrap.php");
		$dir = Configure::read('AMU.directory');
		if (strlen($dir) < 1) $dir = "files";
		...
		$step2 = Router::url(array("controller" => "albums", "action" => "add2"));
		
		if ($formSelector) {
			$successHook = "$('$formSelector').removeAttr('disabled').click();";
		}
		$str .= <<<END
			<br /><br />
				...
				$(document).ready(function(){
					...
					for (var i = 0, max = amuCollection.length; i < max; i++) {
						...
						window['uploader'+i] = new qq.FileUploader({
							...
							onProgress: function(id, fileName, loaded, total) { console.log(loaded + ' de ' + total); }
						});
					}
				});
