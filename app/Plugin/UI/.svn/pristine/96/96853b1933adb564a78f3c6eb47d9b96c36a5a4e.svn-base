...
?>
<div class="association has_and_belongs_to_many <?= $aliasClass ?> base-<?= $baseModelClass ?> <?= $associationClass ?>">
	...
	<div class="actions">
		...
		<? /*<?= $this->Js->popup(
			...
			array("width" => "90%", "callBack" => "fillView", "callBackTargetPath" => ".hasandbelongstomany .entity", 'escape' => false )
		) ?>*/ ?>
		<?= $this->element("UI.Links/has_and_belongs_to_many-add-$addMode", compact("controller", "associatedBasePath", "plugin", "id", "context", "mode", "addOrder")) ?>
		
	</div>
	</h3>
	<div class="entity" data-listindex='<?= count($this->Entity->associationValue($alias)) ?>'>
		<? foreach ($this->Entity->associationValue($alias) as $key => $value) {
			print $this->Entity->form($alias.".".$key, "has_and_belongs_to_many", $mode);
		} ?>
	</div>
</div>