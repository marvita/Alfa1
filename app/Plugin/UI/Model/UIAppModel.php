...
class UIAppModel extends Model {
	...
	public function delete($id = null, $cascade = true) {
      ...
      if ($this->beforeDelete($cascade)) {
        ...
        // print_r($db); die('Test');
        // print_r($this->belongsTo); die('Test');
        // print_r($cascade); die('Test');
			...
			foreach ($results as &$value) {
				foreach ($fillFuncs as $field => $func) {
					if (isset($value[$model][$field])) {
						$value[$model][$field] = call_user_func("Assets::$func", $value[$model][$field]);
	...
	public function saveAssociated($data = null, $options = array()) {
        ...
        // Marta: graba las iniciales del paciente en mayusculas. Este metodo es usado para las fichas y pacientes al agregar y editar
        if(isset($data['Patient']['Iniciales']) && !empty($data['Patient']['Iniciales'])){
            $data['Patient']['Iniciales'] = strtoupper($data['Patient']['Iniciales']);
        }
   
   /*
        if(isset($data['Patient']['FechaNacimiento']) && !empty($data['Patient']['FechaNacimiento'])){
            $data['Patient']['FechaNacimiento'] = Assets::date2sqldate($data['Patient']['FechaNacimiento']);
