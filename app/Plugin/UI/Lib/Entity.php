...
class Entity {
	...
	*/
	...
	protected static function & processAjaxDeleted(&$data, $alias, $association) {
		if (Hash::numeric(array_keys($data))) {
			foreach ($data as $key => &$item) {
				$result = self::processAjaxDeleted($item, $alias, $association);
				if (empty($result)) {
					unset($data[$key]);
				} else {
					$item = $result;
		...
		self::_setEntityPath($alias);
		$model = self::getModel(self::currentPathClass());
		
		// check if there is an __ajax_deleted field
		if (isset($data["__ajax_deleted"]) && $data["__ajax_deleted"]) {
			// has many have to be deleted... habtm must be removed from the array...
			switch ($association) {
				case "hasMany":
					if (isset($data["id"])) {
						//pr("eliminando item modelo $alias id {$data["id"]}");
						$model->delete($data["id"]);
					}
				case "hasAndBelongsToMany":
					self::_revertEntityPath();
					$result = array();
					return $result;
				break;
			}
		...
		foreach ($data as $key => &$sdata) {
			if (array_key_exists($key, $associations)) {
				$dsata = self::processAjaxDeleted($sdata, $key, $associations[$key]);
			}
		}
	...
	public static function & deleteAjaxDeleted(&$data) {
		// we start in the main model and go recursive
		$model = self::getModelScope();
		...
		//if (!isset($data[$baseModel])) {
			...
			foreach ($data as $key => &$sdata) {
				if (array_key_exists($key, $associations)) {
					$sdata = self::processAjaxDeleted($sdata, $key, $associations[$key]);
				}
			}
		//}
