...
class ImageCacheHelper extends AppHelper {
	public $helpers = array('Html' => array('className' => 'UI.UIHtml'), 'Form'  => array('className' => 'UI.UIForm'), 'Js'  => array('className' => 'UI.UIJs'));
	...
	public function resizeImage($path, array $options = array()) {
		...
		return $this->Html->image($this->resize($path, $options));
	}
	
	public function resize($path, array $options = array()) {
		extract($options);
		
		if (substr($path, 0, 4) == "img/" || substr($path, 0, 4) == "img\\") {
			$path = substr($path, 4);
		}
		
		if ( file_exists(IMAGES_THEME . $path) ) {
			//return IMAGES_THEME . $path;
			$path = IMAGES_THEME . $path;
		} elseif ( file_exists(IMAGES . $path) ) {
			//return IMAGES . $path;
			$path = IMAGES . $path;
		} else {
			return IMAGES_THEME . $path.":".IMAGES . $path;
		...
		if (isset($cropZoom) && $cropZoom) {
			$outputdir = "cropzoom";
			$cacheext = "{$newWidth}_{$newHeight}";
		} elseif (isset($canvasFill) && $canvasFill) {
			$outputdir = "canvasfill";
			$cacheext = "{$maxWidth}_{$maxHeight}";
		} else {
			$outputdir = "scale";
			$cacheext = "{$newWidth}_{$newHeight}";
		}
		...
		$output = IMAGES . DS . 'imagecache' . DS . $outputdir . DS . "{$pathinfo["basename"]}_{$cacheext}.png";
		...
		if (!file_exists($output) || (filemtime($path) > filemtime($output))) {
			...
			if (isset($canvasFill) && $canvasFill) {
				...
				//if (in_array($this->_imageInfo['mime'], array('image/gif', 'image/png'))) {
					...
					$alphacolor = imagecolorallocatealpha($dst, 0, 0, 0, 127);
					imagefill($dst, 0,0,$alphacolor);
					imagealphablending($dst, false);
					imagesavealpha($dst, true);
				//}
				$dstOffsetX = ($maxWidth - $newWidth) / 2;
				$dstOffsetY = ($maxHeight - $newHeight) / 2;
			} else {
				// Set up a blank canvas for our resized image (destination)
			...
			imagecolorallocatealpha($src, 0, 0, 0, 127);
			imagealphablending($src, false);
			imagesavealpha($src, true);
			if (false === imagecopyresampled($dst, $src, $dstOffsetX, $dstOffsetY, $offsetX, $offsetY, $newWidth, $newHeight, $width, $height)) {
				return false;
			}
			...
			if ($doSharpen) {
				...
				//	(2) the final size
				$sharpness = $this->_findSharp($height, $newHeight);
		...
		$imagepath = "imagecache/$outputdir/{$pathinfo["basename"]}_{$cacheext}.png";
		$imagepath = str_replace("'", "_", $imagepath);
		
		return $imagepath;
	}
	
	function imageFillCanvas() {
