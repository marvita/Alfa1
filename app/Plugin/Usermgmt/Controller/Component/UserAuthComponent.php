...
class UserAuthComponent extends Component {
	...
	function beforeFilter(&$c) {
		...
		$appAccess = "App/".$action;
		$allControllers=$this->ControllerList->getControllerWithMethods();
		$errorPage=false;
		if (!in_array($access, $allControllers) && !in_array($appAccess, $allControllers)) {
			$errorPage=true;
		}
		
		if ((empty($pageRedirect) || $actionUrl!='users/login') && !$requested && !in_array($actionUrl, $permissionFree) && !$errorPage && !$this->isAdmin()) {
			App::import("Model", "Usermgmt.UserGroup");
			$userGroupModel = new UserGroup;
			if (!$this->isLogged()) {
				if (! ($userGroupModel->isGuestAccess($controller, $action) || 
					(!in_array($controller, array("users", "user_groups")) && $userGroupModel->isGuestAccess("app", $action)) ) ) {
				...
				if (! ($userGroupModel->isUserGroupAccess($controller, $action, $this->getGroupId()) || 
					(!in_array($controller, array("users", "user_groups")) && $userGroupModel->isUserGroupAccess("app", $action, $this->getGroupId())) ) ) {
					$c->log('permission: actionUrl-'.$actionUrl, LOG_DEBUG);
					$c->Session->write('permission_error_redirect','/users/login');
